...
# Stores username -> (IP, listening port)
connected_clients = {}

# Function that handle all the clients created at the runtime
def handle_client(client_socket, ip, username):
    try:
        while True:
            ...
            if request.startswith("LISTEN_PORT:"):
                ...
                user_list = ""
                # Append all the users except the same user to the user list and return it to the client when requested
                for u, data in connected_clients.items():
                    if u != username:
                        user_list += f"{u} {data[0]} {data[1]}\n"

                # Send list if users available
                if user_list:
                ...
                # When client is disconnected remove it from the list of available/online users
    ...
    # Exception handling in case of unusual response from the client
...
def start_server():
    ...
    # Allow 5 users in the queue to wait
...
# Start the server and it will be ready to handle multiple clients
start_server()
